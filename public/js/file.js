...
/* This function sorts the table rows based on the column selected */
function sortTableColumn(column, asc = true) {
    const ascDesc = asc ? 2 : 1; // whether we want to sort ascending or descending
...
function displayPage(number) {
    ...
    let low = (number - 1) * 5; // lower index
    let high = tableRows < 5 * number ? tableRows : 5 * number - 1; // upper index
    for (let i = 0; i < tableRows; i++) {
        if (rows[i] == undefined) return;
        if ((low <= i && i <= high) || i == 0) {
            // first row is in range always
            rows[i].style.display = '';
        } else {
...
function pagination() {
    let rows = 5; // num of rows we want to display
    let tableRows = document.querySelectorAll('table tr').length; // num of rows
...
// This function adds event listeners to the page buttons
function getPage() {
